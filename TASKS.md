...
Approximate image sizes (x86_64). Sizes are cumulative — each row adds to the rows above it.

### Base image (always included)

| Layer | Adds | Running Total |
|-------|------|---------------|
| Ubuntu 24.04 base | ~78 MB | ~78 MB |
| Base APT packages (git, curl, ripgrep, bat, fzf, zoxide, fd, etc.) | ~120 MB | ~198 MB |
| External APT repos + delta + yq (gh, eza, gnupg, delta, yq) | ~100 MB | ~298 MB |
| Binary tools (lazygit, xh, yazi, starship, gh-dash, glow) | ~95 MB | ~393 MB |
| User setup + config files | ~1 MB | ~394 MB |

**Base image total: ~394 MB**

### First-run setup (setup.sh choices)
...
| **Full editors** | |
| fresh | ~10 MB |
| helix (includes runtime) | ~80 MB |
| nvim (Neovim) | ~45 MB |
| **SDKs** | |
| Node.js (via nvm, LTS) | ~90 MB |
| Python (via uv) | ~35 MB |
| Go | ~500 MB |
| .NET (LTS) | ~800 MB |

A typical setup (Claude Code + Node.js + one editor) adds ~400-420 MB on top of the base, bringing the total to **~800 MB**.

A maximal install (all AI tools, all editors, all SDKs) could reach **~2.4 GB**.
